            ...
            <div className="mx-auto flex max-w-4xl flex-col gap-8 md:flex-row">
                {steps.map((s, i) => (
                    <div key={s.step} className="relative flex-1 text-center">
                        <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl border border-zinc-200 bg-white">
                        ...
                        <h3 className="mb-2 mt-2 text-xl font-semibold text-zinc-900">{s.title}</h3>
                        <p className="text-sm leading-relaxed text-zinc-600">{s.description}</p>

                        {i < steps.length - 1 && (
                            <div className="absolute right-0 top-8 hidden h-[2px] w-8 translate-x-1/2 bg-zinc-200 md:block" />
                        )}
                    </div>
                ))}
            </div>
